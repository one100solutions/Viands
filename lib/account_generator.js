(function() {
   ...
   //omitPhone = [8277564501, 8970707712, 9986787295, 7411487928, 9482532445, 7259281007];
    ...
    var finalAccounts = {};
    var NameFinder = require('./menuHelper').HashMap;
...
//                    console.log('Diff', curDate.diff(dbDate, 'days'));
                    if (curDate.dayOfYear() === dbDate.dayOfYear()) {
                       order.items = order.items.map(function  (item) {
                            ...
                            item.amount = Name[item.id].order;
...
//            mail();
    ...
    exports.AccountGenerator = function (dateForAccount, callback) {
        ...
        console.log('stat', ordered.length, 's', credited.length)
         ...
         console.log('D',curDate.format())

         NameFinder(function  (err, hashMap) {
             // body...
             Name = hashMap;
    ...
    function resolve (i, callback) {
        if(i == 2) {
           // calculator(callback)
           SimpleCalculator(callback);
            i=0;
        }
    }

    function calculator(callback) {
        var _phone, _totalCredited, _totalOrdered, i, j, len, newObj, reduceHelper, totalCreditSum, totalOrderSum, uniqueObjectCredit,
         uniqueObjectOrder, uniquePhone, uniquePhoneCredited, uniquePhoneOrdered;
...
//        createCsv(finalAccounts)
    ...
    function SimpleCalculator (callback) {
        // body...
        console.log("Credits", credited[0], "Orders", ordered[0]);
        var tot_c = 0,
            tot_o = 0;
        credited.forEach(function  (credits) {
            tot_c += credits.amount;
        })

        ordered.forEach(function  (orders) {
            tot_o += orders.total;
        })

        finalAccounts = {};
        finalAccounts.orders = ordered;
        finalAccounts.credits = credited;
        finalAccounts.totalOrdered = tot_o;
        finalAccounts.totalCredited = tot_c;
        callback(null,finalAccounts);
    }
