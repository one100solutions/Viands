(function() {
  var CreditSumHistory, Credits, OrderSumHistory, Orders, _, async, credit, credited, creditsString, fastCsv, fs, json2csv, createCsv, mailer, message, moment, mongoose, omitPhone, order, ordered, ordersString;
  ...
  createCsv = function(Account) {
    ...
    json2csv({
      ...
      fields: ['phone', 'totalOrdered', 'totalCredited']
    }, function (err, csv) {
        if (err) {
          console.log('Error occured', err);
        }
        else {
          console.log('Csv v', csv)
  ...
  /*  mailString = mailString2 = "";
    ...
    mailer('ashakdwipeea@gmail.com', final, function(err, response) {
      if (err) {
        console.log(err);
      }
  ...
  async.parallel([order, credit], function(err, results) {
    ...
    return console.log(finalAccounts);
  });
  ...
  function mail () {
    ...
    fs.readFile("Account.csv", function(err, data) {
      ...
      mailer('ashakdwipeea@gmail.com','Find the account statement', data, function (Err, Success) {
